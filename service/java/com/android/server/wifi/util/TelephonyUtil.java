...
public class TelephonyUtil {
    ...
    public static Pair<String, String> getSimIdentity(TelephonyManager tm,
        ...
        //}
        int[] subId = SubscriptionManager.getSubId(config.enterpriseConfig.getSimNum());
        if (subId == null || !SubscriptionManager.isValidSubscriptionId(subId[0])) {
            Log.e(TAG, "Failed to get valid subId");
            return null;
        }
        String imsi = tm.getSubscriberId(subId[0]);
        String mccMnc = "";
        if (tm.getSimState(config.enterpriseConfig.getSimNum()) == TelephonyManager.SIM_STATE_READY) {
            mccMnc = tm.getSimOperator(subId[0]);
        }
